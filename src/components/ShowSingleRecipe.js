...
export default function ShowSingleRecipe() {
    ...
    useEffect(() => {
        Axios.get(`http://localhost:3002/api/getFromId/${recipeId}`).then(
            (data) => {
    ...
    useEffect(() => {
        Axios.get(
            `http://localhost:3002/api/getIngredientsFromId/${recipeId}`
        ).then((data) => {
        ...
        return (
            <div key={key} className="header zindex">
                ...
                <div className="single-recipe">
                    <img
                        src={`/images/recipe/${recipeId}.jpg`}
                        alt={recipe.title}
                    ...
                    <p>
                        ...
                        <span>
                            ...
                            {recipe.cooking_time >= 60
                                ? convertTime(recipe.cooking_time)
                                : recipe.cooking_time}{" "}
                            minutes
                        </span>
                    </p>
